function check_form(){
    ...
    if (document.getElementById('latitude').value >= 90 || document.getElementById('latitude').value <= -90){
        document.getElementById('latitude_check_msg').innerHTML = `Latitude should be in -90 ~ 90, current is ${document.getElementById('latitude').value}`;
        valid = false;
    }
    // longitude
    if (/^-?[0-9]{1,}.[0-9]{1,}$/.test(document.getElementById('longitude').value) == false && longitude >= -180 && latitude <= 180){
        document.getElementById('longitude_check_msg').innerHTML = "Only allow float numbers !!";
        valid = false;
    }
    if (document.getElementById('longitude').value >= 180 || document.getElementById('longitude').value <= -180){
        document.getElementById('longitude_check_msg').innerHTML = `Longitude should be in -180 ~ 180, current is ${document.getElementById('longitude').value}`;
        valid = false;
    }
